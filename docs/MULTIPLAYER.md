...
3. เพื่อนเปิดลิงก์ → ตั้งชื่อ → เข้าห้อง (ไม่ต้องสมัครสมาชิก) ได้แมวที่ว่างแบบสุ่ม แล้ว**เลือกแมวในห้องรอ**
   (เปลี่ยนได้จนเกมเริ่ม รวมตอนรอเล่นอีกรอบ · แมวของคนอื่นจางพร้อมชื่อ · แมวของบอทแตะเอามาได้ — DECISIONS 052)
4. เจ้าของห้องเห็นรายชื่อ เติมบอทในที่นั่งว่าง (เลือกแค่ระดับ ง่าย/ปกติ · นิสัยและแมวสุ่ม) แล้วกด "เริ่มเกม" (ต้องมีอย่างน้อย 2 ที่นั่ง)
...
Client → Server
- `hello {name, cat?, token?}` นั่งที่ว่าง (ไม่ส่ง cat = สุ่มแมวที่ว่าง) / กลับที่นั่งเดิมด้วย token / ถ้าเต็มหรือเริ่มเกมแล้ว = ผู้ชม
- `action {action}` · `emote {id}` · `ping` · `updateMe {name, cat}` (ในห้องรอ — แมวของบอทได้เลย บอทสุ่มใหม่; แมวของคนอื่น = `catTaken`) · `leave`
- หลังจบเกม: `ready {ready}` = อยากเล่นอีกรอบ (GAME_RULES §13) · `updateMe` ใช้ได้ทั้งห้องรอและหลังจบเกม
- เฉพาะเจ้าของห้อง (ห้องรอ หรือหลังจบเกม): `addBot {bot: {difficulty, personality?}}` (ไม่ส่งนิสัย = สุ่ม · แมวสุ่มจากที่ว่างเสมอ) · `removeSeat {seatId}` (บอท หรือคนที่หลุดไปแล้ว) ·
...
- **จังหวะเกม (DECISIONS 051):** server ไม่ต้องรอ client — มันคำนวณเวลาที่จอใช้แสดงเหตุการณ์ (`showTiming` ใช้สูตรเดียวกับหน้าจอ
  ด้วยภาษาที่อ่านช้าที่สุด และความเร็วของห้อง) · บอทเดินหลังจอแสดงครบ · **คนก็เล่นไม่ได้จนกว่า popup ล่าสุดหมดเวลา**
  (`openAt` — ส่งมาใน `view.openInMs` หน้าจอปิดปุ่มไว้ "ทุกคนยังอ่านประกาศอยู่…" · ส่งมาก่อน = `room.error.notYet`, เผื่อ 250 ms)
  · ตาราง "จำนวนตัวอักษรของ popup" อยู่ใน protocol (server ไม่มีพจนานุกรม) เทสต์ `apps/web/test/pacing.test.ts` เช็กว่าตรงกับ th/en.json
  · ความเร็วห้อง `setSpeed` (เจ้าของห้อง) · ห้องเก่าข้าม deploy = ปกติ
  แล้วให้บอท/การลงใบเหมียวใบสุดท้ายอัตโนมัติรอจนเลยเวลานั้นก่อน + เวลาคิด 0.6–1.2 วิ × ความเร็วห้อง
